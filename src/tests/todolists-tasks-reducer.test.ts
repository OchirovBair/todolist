import { v1 } from "uuid";
import { tasksReducer, TasksType } from "features/TodolistsList/tasksSlice";
import {
...
test("after adding new todolist should added empty array of tasks", () => {
  ...
  const action = todolistsActions.addTodolist({todolist: { id: v1(), title: "new todolist", addedDate: "", order: 0 }});
...
test("after deleting todolist with id=todolistId1, array of tasks with key=todolistId1 should be deleted", () => {
  ...
  const endTasksState = tasksReducer(startTasks, todolistsActions.removeTodolist({todoId: todolistId1}));
  const endTodolistsState = todolistsReducer(startTodolists, todolistsActions.removeTodolist({todoId: todolistId1}));
