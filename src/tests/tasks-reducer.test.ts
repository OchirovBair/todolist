...
test("correct task should be deleted from correct array", () => {
  ...
  const action = tasksActions.removeTask({taskId: taskId, todolistId: todolistId2});
...
test("title of specified task should be changed", () => {
  const action = tasksActions.updateTask({todolistId: todolistId2, model: { title: "onion" }, taskId: "2"});
...
test("task should be added for todo", () => {
  ...
  const action = tasksActions.setTasks({tasks: tasks, todolistId: todolistId2});
...
test('when task entity status if "loading" then entityStatus should be "loading"', () => {
  const action = tasksActions.updateTask({taskId: "1", todolistId: todolistId1, model: { entityStatus: "loading" }});
